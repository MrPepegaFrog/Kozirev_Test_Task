import './filters.scss';
import searchIco from '../../shared/assets/ico/search.svg';
import { debounce } from '../../shared/utils/debounce/Debounce';
export class Filters {
  constructor({ data, onFilterChange }) {
    this.data = data;
    this.onFilterChange = onFilterChange;
    this.activeCategory = 'All';
    this.searchQuery = '';

    this.categories = [
      { label: 'All', type: 'all' },
      { label: 'Marketing', type: 'marketing' },
      { label: 'Managment', type: 'management' },
      { label: 'HR&recruting', type: 'hr_recruting' },
      { label: 'Design', type: 'design' },
      { label: 'Development', type: 'development' },
    ];

    this.debouncedSearch = debounce((query) => {
      this.searchQuery = query;
      this.notifyChange();
    }, 300);
  }

  getCount(type) {
    if (type === 'all') return this.data.length;
    return this.data.filter((item) => item.type === type).length;
  }

  create() {
    const container = document.createElement('div');
    container.className = 'filters';

    const searchWrapper = document.createElement('div');
    searchWrapper.className = 'filters__search-wrapper';

    const searchIcon = document.createElement('div');
    searchIcon.className = 'filters__search-icon';
    searchIcon.innerHTML = `
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
<g clip-path="url(#clip0_82282_97)">
<path fill-rule="evenodd" clip-rule="evenodd" d="M7.25016 2.16634C4.44272 2.16634 2.16683 4.44223 2.16683 7.24967C2.16683 10.0571 4.44272 12.333 7.25016 12.333C8.65276 12.333 9.9216 11.7658 10.842 10.8467C11.7642 9.92589 12.3335 8.6549 12.3335 7.24967C12.3335 4.44223 10.0576 2.16634 7.25016 2.16634ZM0.833496 7.24967C0.833496 3.70585 3.70634 0.833008 7.25016 0.833008C10.794 0.833008 13.6668 3.70585 13.6668 7.24967C13.6668 8.78248 13.1287 10.1905 12.2321 11.2939L14.9711 14.0278C15.2317 14.2879 15.2321 14.7101 14.972 14.9706C14.7119 15.2312 14.2898 15.2316 14.0292 14.9715L11.2888 12.2362C10.1863 13.13 8.78039 13.6663 7.25016 13.6663C3.70634 13.6663 0.833496 10.7935 0.833496 7.24967Z" fill="#9A9CA5"/>
</g>
<defs>
<clipPath id="clip0_82282_97">
<rect width="16" height="16" fill="white"/>
</clipPath>
</defs>
</svg>
  `;
    const searchInput = document.createElement('input');
    searchInput.type = 'text';
    searchInput.className = 'filters__search';
    searchInput.placeholder = 'Search courses...';
    searchInput.value = this.searchQuery;

    searchInput.addEventListener('input', (e) => {
      this.debouncedSearch(e.target.value);
    });
    searchWrapper.appendChild(searchIcon);
    searchWrapper.appendChild(searchInput);
    const buttonsContainer = document.createElement('div');
    buttonsContainer.className = 'filters__buttons';

    this.categories.forEach((category) => {
      const btn = document.createElement('button');
      const isActive = this.activeCategory === category.type ? 'active' : '';
      btn.className = `filter-btn ${isActive}`;

      btn.innerHTML = `
       
       <span class="filter-btn__text">${category.label}</span>
       <sup class="filter-btn__count">${this.getCount(category.type)}</sup>
       
       
        
      `;

      btn.addEventListener('click', () => {
        this.activeCategory = category.type;
        this.notifyChange();
        this.updateButtonsState(buttonsContainer);
      });

      buttonsContainer.appendChild(btn);
    });

    container.appendChild(searchWrapper);
    container.appendChild(buttonsContainer);

    return container;
  }

  #updateButtonsState(container) {
    const buttons = container.querySelectorAll('.filter-btn');
    buttons.forEach((btn, index) => {
      const type = this.categories[index].type;
      if (this.activeCategory === type) {
        btn.classList.add('active');
      } else {
        btn.classList.remove('active');
      }
    });
  }

  #notifyChange() {
    if (this.onFilterChange) {
      this.onFilterChange({
        category: this.activeCategory,
        search: this.searchQuery,
      });
    }
  }
}
